...
- July 23, 2025. **AUTHENTIC SENTIMENT ANALYSIS**: Completely overhauled Market Sentiment Analysis section with live data sources - NewsAPI for news sentiment, Reddit r/Bitcoin API for social sentiment, CoinPaprika for on-chain metrics, Fear & Greed Index for trading activity. All cards now clickable to view source data, removed all mock/fake data, implemented RSS fallback for news when APIs unavailable, updated color scheme for Treasury widget (green for rate decreases, red for increases)
- July 23, 2025. **CRYPTO CATALYSTS TRACKER**: Added comprehensive crypto events tracker to legislation page monitoring 9 key July 2025 catalysts including White House crypto policy report (90% probability), Solana ETF approval (70%), GENIUS Act implementation (100%), SEC meetings, FOMC decisions, DeFi launches, and Bitcoin strategic reserve developments. Features probability scoring, impact assessment, actionable next steps, market impact analysis, and risk factors based on authentic web sources and X sentiment data
